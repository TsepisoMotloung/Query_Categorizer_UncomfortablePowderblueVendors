...
class AdvancedQueryCategorizer:
    def __init__(self):
        ...
        self.category_patterns = {
            "Date of Birth Issues": {
                'keywords': [
                    'date of birth', 'dob', 'birth date', 'birthday', 'birth not provided', 
                    'birth certificate', 'date missing', 'birth missing'
                ],
                'regexes': [r'(?:date|d\.?o\.?b\.?).*(?:birth|born).*(?:miss|not|incorrect)', 
                           r'(?:birth|born).*(?:date|d\.?o\.?b\.?).*(?:miss|not|incorrect)']
            },
            "ID Related": {
                'keywords': [
                    'identity', 'id', 'identification', 'identity number', 'id number', 
                    'id document', 'identity document', 'id card', 'id incorrect', 
                    'identity incorrect', 'id not provided', 'identity not provided',
                    'id verification', 'id validity', 'identity validity', 'incomplete id'
                ],
                'regexes': [r'id(?:entity)?.*(?:num|no|card|doc).*(?:miss|not|incorrect|invalid|incomplete)',
                           r'(?:miss|not|incorrect|invalid|incomplete).*id(?:entity)?']
            },
            "Beneficiary Issues": {
                'keywords': [
                    'beneficiary', 'beneficiaries', 'nominee', 'nominees', 'benefit recipient',
                    'benefit allocation', 'benefit distribution', 'benefit assignment'
                ],
                'regexes': [r'beneficiar(?:y|ies).*(?:miss|not|incorrect|select|relationship)',
                           r'(?:miss|not|incorrect).*beneficiar(?:y|ies)']
            },
            "Age Qualification": {
                'keywords': [
                    'age', 'aged', 'aging', 'overage', 'over age', 'age limit', 'age restriction',
                    'age requirement', 'age qualification', 'qualify', 'qualification'
                ],
                'regexes': [r'age.*(?:limit|restrict|require|qualif|over)',
                           r'(?:not|doesn\'?t).*qualify',
                           r'over.*age', r'too.*old']
            },
            "Premium Issues": {
                'keywords': [
                    'premium', 'premiums', 'payment', 'payments', 'fee', 'fees', 'cost',
                    'monthly payment', 'amount', 'rate', 'pricing', 'price'
                ],
                'regexes': [r'premium.*(?:incorrect|wrong|invalid|miss|not)',
                           r'rider.*premium', r'payment.*(?:incorrect|wrong|invalid)']
            },
            "Signature Issues": {
                'keywords': [
                    'signature', 'sign', 'signing', 'signed', 'autograph', 'endorsement',
                    'mark', 'signed document', 'authorize', 'authorization'
                ],
                'regexes': [r'sign(?:ature)?.*(?:miss|not|incorrect|invalid|incomplete)',
                           r'(?:miss|not).*sign(?:ature)?', r'no.*sign(?:ature)?']
            },
            "Bank Confirmation": {
                'keywords': [
                    'bank', 'banking', 'account', 'confirmation', 'conformation', 'verify',
                    'verification', 'validate', 'validation', 'proof', 'statement', 'stop order'
                ],
                'regexes': [r'bank.*(?:confirm|proof|statement|miss|not|verification)',
                           r'(?:miss|not).*bank.*(?:confirm|proof|statement)',
                           r'stop\s*order']
            },
            "Territorial Limits": {
                'keywords': [
                    'territorial', 'territory', 'region', 'location', 'residence', 'residency',
                    'proof of residence', 'resident', 'address', 'jurisdiction'
                ],
                'regexes': [r'territor(?:y|ial).*(?:limit|restrict)',
                           r'proof.*(?:residence|address|location)',
                           r'lesotho.*residence']
            },
            "Affordability": {
                'keywords': [
                    'afford', 'affordability', 'budget', 'income', 'affordable', 'financial',
                    'finance', 'capability', 'capacity', 'means'
                ],
                'regexes': [r'afford(?:ability)?', r'financial.*capacity']
            },
            "Rider Benefits": {
                'keywords': [
                    'rider', 'riders', 'benefit', 'benefits', 'coverage', 'add-on', 'additional',
                    'option', 'molebe', 'supplemental'
                ],
                'regexes': [r'rider.*(?:benefit|cover|option|molebe)',
                           r'(?:miss|not|incorrect).*rider',
                           r'rider.*(?:not|miss|incorrect)']
            },
            "Deduction Issues": {
                'keywords': [
                    'deduction', 'deduct', 'deducted', 'withhold', 'withholding', 'withdraw',
                    'withdrawal', 'payroll', 'employment number'
                ],
                'regexes': [r'deduct(?:ion)?.*(?:miss|not|incorrect|invalid)',
                           r'employment.*number', r'payroll.*(?:miss|not|incorrect)']
            },
            "Declaration Issues": {
                'keywords': [
                    'declaration', 'declare', 'statement', 'attestation', 'checklist',
                    'check list', 'form completion', 'disclosure'
                ],
                'regexes': [r'declarat(?:ion)?.*(?:miss|not|incomplete|section)',
            ...
            "KYC Issues": {
                ...
                'regexes': [r'k\.?y\.?c\.?.*(?:miss|not|incomplete|form)',
        ...
        for category, info in self.category_patterns.items():
            info['compiled_regexes'] = [re.compile(pattern, re.IGNORECASE) for pattern in info['regexes']]
    ...
    def preprocess_text(self, text):
        ...
        tokens = [self.lemmatizer.lemmatize(token) for token in tokens if token.isalpha() and token not in self.stopwords]
        return ' '.join(tokens)
    ...
    def categorize_query(self, query, verbose=False):
        ...
        if top_score >= 3:
            if verbose:
                print(f"Categorized '{query}' as '{top_category}' with score {top_score}")
            return top_category

        # 2. For ambiguous cases, use semantic similarity with TF-IDF
        if top_score < 3 or (len(sorted_categories) > 1 and sorted_categories[1][1] >= top_score - 1):
            # Prepare vectorizer
            ...
            if best_semantic_category[1] > 0.2:  # Threshold for semantic similarity
                if verbose:
                    print(f"Semantically categorized '{query}' as '{best_semantic_category[0]}' with similarity {best_semantic_category[1]}")
                return best_semantic_category[0]

        # 3. Special case handling for common patterns
        if "premium" in raw_query and any(term in raw_query for term in ["incorrect", "wrong"]):
        ...
        if "age" in raw_query and any(term in raw_query for term in ["over", "not qualify"]):
...
class AdvancedQueryCategorizationTool:
    ...
    def setup_main_tab(self, tab):
        ...
        ttk.Label(input_frame, text="Select Input Directory with Query Files:").pack(pady=(10, 5))
        ...
        self.input_path_display = ttk.Label(input_frame, text="", font=("Arial", 9, "italic"))
        self.input_path_display.pack(pady=5)
        ...
        ttk.Label(output_frame, text="Select Output Directory for Categorized Files:").pack(pady=(10, 5))
        ...
        self.output_path_display = ttk.Label(output_frame, text="", font=("Arial", 9, "italic"))
        self.output_path_display.pack(pady=5)
        ...
        ttk.Checkbutton(options_frame, text="Enable detailed logging", variable=self.verbose_var).pack(pady=5, padx=10, anchor=tk.W)

        # Test on sample option
        self.test_sample_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="Run accuracy test after processing", variable=self.test_sample_var).pack(pady=5, padx=10, anchor=tk.W)
        ...
        # Status Display
        self.status_label = ttk.Label(process_frame, text="Ready", font=("Arial", 9))
    ...
    def setup_stats_tab(self, tab):
        ...
        # Default text
        self.stats_text.insert(tk.END, "Statistics will appear here after processing files.")
    ...
    def setup_test_tab(self, tab):
        ...
        ttk.Label(test_frame, text="Enter a query to test categorization:").pack(pady=(10, 5))
        ...
        # Test button
        test_button = ttk.Button(test_frame, text="Test Categorization", command=self.test_categorization)
        ...
        test_scrollbar = ttk.Scrollbar(result_frame, command=self.test_result_text.yview)
        ...
        batch_button = ttk.Button(batch_frame, text="Run Batch Tests", command=self.run_batch_tests)
        batch_button.pack(pady=10)

    def select_input_directory(self):
        """Handle input directory selection"""
        self.input_directory = filedialog.askdirectory(title="Select Directory with Query Files")
        if self.input_directory:
            self.input_path_display.config(text=self.input_directory)
            self.check_process_readiness()
    ...
    def select_output_directory(self):
        ...
        self.output_directory = filedialog.askdirectory(title="Select Output Directory for Categorized Files")
        if self.output_directory:
            self.output_path_display.config(text=self.output_directory)
            self.check_process_readiness()
    ...
    def process_files(self):
        ...
        try:
            ...
            if not excel_files:
                messagebox.showwarning("No Files", "No Excel files found in the selected directory.")
            ...
            for i, file in enumerate(excel_files):
                try:
                    ...
                    # Check if 'Query' column exists
                    if 'Query' not in df.columns:
                        messagebox.showwarning("Column Missing", f"No 'Query' column in {file}. Skipping.")
                        continue
                    ...
                    df['Category'] = df['Query'].apply(lambda q: self.categorizer.categorize_query(q, verbose))
            ...
            # Final status update
            self.status_label.config(
                text=f"Processed {processed_count} files successfully!", foreground="green")
            messagebox.showinfo(
                "Complete",
    ...
    def update_statistics(self, category_counts, combined_df):
        ...
        for category, count in sorted(category_counts.items(), key=lambda x: x[1], reverse=True):
            ...
            self.stats_text.insert(tk.END, f"{category}: {count} ({percentage:.1f}%)\n")
        ...
        if 'Original_Category' in combined_df.columns:
            ...
            # Calculate accuracy
            matches = sum(combined_df['Original_Category'] == combined_df['Category'])
            accuracy = (matches / len(combined_df)) * 100 if len(combined_df) > 0 else 0

            self.stats_text.insert(tk.END, f"Overall Accuracy: {accuracy:.2f}%\n")
            self.stats_text.insert(tk.END, f"Matching Categories: {matches}/{len(combined_df)}\n\n")
            ...
            for category in combined_df['Original_Category'].unique():
                ...
                category_matches = sum(category_df['Original_Category'] == category_df['Category'])
                category_accuracy = (category_matches / len(category_df)) * 100 if len(category_df) > 0 else 0

                self.stats_text.insert(tk.END, f"{category}: {category_accuracy:.2f}% "
    ...
    def run_batch_tests(self):
      ...
      test_cases = [
          ...
          ("Premium incorrect - spouse riders incorrect because of age", "Premium Issues"),
          ("Beneficiary not selected", "Beneficiary Issues"),
          ("RELATIONSHIP-RELATIONSHIP OF PREMIUM PAYER AND BENEFICIARY NOT PROVIDED", "Beneficiary Issues"),
          ...
          ("Client staying in Lesotho - requires proof of residence", "Territorial Limits"),
      ...
      for query, expected, actual, is_match in results:
          ...
          self.test_result_text.insert(tk.END, f"{result_marker} ", f"result_{result_color}")
          self.test_result_text.insert(tk.END, f"Query: {query}\n")
      ...
      self.test_result_text.tag_configure("result_green", foreground="green", font=("Arial", 9, "bold"))
      self.test_result_text.tag_configure("result_red", foreground="red", font=("Arial", 9, "bold"))
    ...
    def test_accuracy(self, df):
      ...
      all_categories = sorted(list(set(original_categories) | set(predicted_categories)))
      ...
      ttk.Label(frame, text="Accuracy Test Results", font=("Arial", 16, "bold")).pack(pady=10)
      ttk.Label(frame, text=f"Overall Accuracy: {accuracy:.2f}% ({matches}/{total})").pack(pady=5)

      # Add per-category stats
      ttk.Label(frame, text="Per-Category Stats:", font=("Arial", 12, "bold")).pack(pady=(15, 5), anchor=tk.W)
      ...
      for category in original_categories:
          ...
          category_accuracy = (category_matches / category_total) * 100 if category_total > 0 else 0

          results_text.insert(tk.END, f"{category}:\n")
          results_text.insert(tk.END, f"  Accuracy: {category_accuracy:.2f}% ({category_matches}/{category_total})\n")
